...
class SerialMonitor():
    ...
    def setupRightFrame(self):
        ...
        # Creating scrolled text area
        self.serialOutput = st.ScrolledText(self.rightFrame, width = 100, height = 28, font = ("Times New Roman", 12))

        # self.serialInBuff = tk.StringVar() text = self.serialInBuff
        self.serialInput = st.ScrolledText(self.rightFrame, width = 90, height = 2, font = ("Times New Roman", 12))
        self.serialSend = tk.Button(self.rightFrame, text="Send", width=10, height = 2, command=lambda: self.sendSerial())

        self.rightFrame.grid(row = 0, column = 1, padx = 10, pady = 5)
        self.serialMonitorLabel.grid(row = 0, column = 0)

        self.serialOutput.grid(row = 0, column = 0, columnspan=2)
        self.serialInput.grid(row = 1, column = 0, columnspan=1)
        self.serialSend.grid(row = 1, column = 1, columnspan=1)
    ...
    def connectToSerialPort(self):
        ...
        if (self.portName is None):
            ...
            return
        try:
            self.serialPort = serial.Serial(port = self.portName, baudrate = 1000000, timeout=1)
    ...
    def sendSerial(self):
        # buff = self.serialInBuff.get()       
        buff = str(self.serialInput.get(1.0, tk.END)).replace("\n", "") + "\r\n"
    ...
    def readSerialPort(self):
        ...
        while self.serialConnect:
            if (self.serialPort.in_waiting > 0):
                ...
                self.serialOutput.insert("end", (data.decode('utf-8', "ignore")).replace('\r', '')) 
